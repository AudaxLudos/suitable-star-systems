...
public class UGPhase4 extends HubMissionWithBarEvent {
    ...
    protected boolean create(MarketAPI createdAt, boolean barEvent) {
        ...
        connectWithDaysElapsed(Stage.WAIT, Stage.TALK_TO_PERSON, 1f);
        connectWithGlobalFlag(Stage.TALK_TO_PERSON, Stage.KILL_FLEET, "$UGPhase4_remnantHere");
    ...
    public void addDescriptionForNonEndStage(TooltipMakerAPI info, float width, float height) {
        ...
        if (currentStage == Stage.WAIT) {
            info.addPara(person.getNameString() + " is recovering wait for " + person.getHimOrHer() + " to fully recover and contact you", opad);
        } else if (currentStage == Stage.TALK_TO_PERSON) {
            info.addPara("Talk to " + person.getNameString() + " and tell " + person.getHimOrHer() + " about the decrypted data core.", opad);
    ...
    public boolean addNextStepText(TooltipMakerAPI info, Color tc, float pad) {
        // Color h = Misc.getHighlightColor();
        if (currentStage == Stage.WAIT) {
            info.addPara("Wait until " + person.getNameString() + " fully recovers", tc, pad);
            return true;
        } else if (currentStage == Stage.TALK_TO_PERSON) {
            info.addPara("Talk to " + person.getNameString() + " about the decrypted data core", tc, pad);
            return true;
        } else if (currentStage == Stage.KILL_FLEET) {
